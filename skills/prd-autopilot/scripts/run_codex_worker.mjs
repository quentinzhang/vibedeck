...
async function main() {
  ...
  else throw new Error(`Invalid --mode: ${mode} (expected danger|full-auto)`);
  ...
  await new Promise((resolve) => {
    const child = spawn(codexCmd, codexArgs, { stdio: ['pipe', 'inherit', 'inherit'] });
    child.stdin.write(promptText);
    child.stdin.end();

    child.on('exit', async (code) => {
      const n = Number.isFinite(code) ? code : 1;
      await fs.writeFile(exitPath, `${n}\n`, 'utf8').catch(() => {});
      resolve();
    });
  });
}
