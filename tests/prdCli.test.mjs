...
async function setupTempCliRepo(root) {
  ...
  await fs.copyFile(path.join(process.cwd(), 'bin', 'vbd.mjs'), path.join(root, 'bin', 'vbd.mjs'));
...
async function createTempRepoDir(prefix = 'prd-cli-repo-') {
...
test('prd CLI supports project new, project list, new, and list pending (with hub symlinked skills)', async () => {
  ...
  const mappedRepo = await createTempRepoDir();
  ...
  // 1) Create project non-interactively
  {
    ...
    assert.equal(registry.projects.p1.repoPath, mappedRepo);
  }
  ...
  // 2) Project list
  {
    ...
    assert.equal(res.status, 0, (res.stderr || '') + (res.stdout || ''));
    assert.match(res.stdout || '', new RegExp(`- p1: ${mappedRepo.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`));
  }
...
test('prd CLI supports move + archive and enforces unique ids per project', async () => {
  ...
  // Create project
  {
    const res = spawnSync(
      process.execPath,
      [prdBin, 'project', 'new', '--hub', tmp, '--project', 'p1', '--repo-path', mappedRepo, '--non-interactive'],
      { encoding: 'utf8' },
    );
    assert.equal(res.status, 0, (res.stderr || '') + (res.stdout || ''));
  }
...
test('prd CLI exposes roll as the preferred supervisor command and keeps autopilot compatibility', async () => {
  ...
  assert.match(helpRes.stdout || '', /Project registry commands:/i);
  assert.match(helpRes.stdout || '', /Card lifecycle commands:/i);
  assert.match(helpRes.stdout || '', /Supervisor commands:/i);
  assert.match(helpRes.stdout || '', /vbd roll <dispatch\|reconcile\|tick>/);
  assert.match(helpRes.stdout || '', /vbd autopilot <dispatch\|reconcile\|tick> .*legacy alias/i);
  assert.match(helpRes.stdout || '', /--agent-invoke exec\|prompt/i);
  assert.match(helpRes.stdout || '', /Defaults\s+Runner=process; Codex uses exec; Claude uses prompt unless runner=process/i);
  assert.match(helpRes.stdout || '', /headless\s+Legacy alias of exec/i);
  assert.match(helpRes.stdout || '', /print\s+Legacy alias of exec/i);
  assert.match(helpRes.stdout || '', /process \+ prompt\s+Invalid because prompt mode requires a TTY/i);
  assert.match(helpRes.stdout || '', /Prefer vbd roll .*autopilot only for compatibility/i);
...
test('prd roll reads autopilot defaults from vbd.config.json and explicit flags override them', async () => {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'prd-cli-roll-config-'));
  await setupTempCliRepo(tmp);
  await fs.writeFile(
    path.join(tmp, 'vbd.config.json'),
    JSON.stringify(
      {
        hubRoot: '.',
        autopilot: {
...
test('prd roll accepts agent defaults from vbd.config.json', async () => {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'prd-cli-roll-agent-config-'));
  await setupTempCliRepo(tmp);
  await fs.writeFile(
    path.join(tmp, 'vbd.config.json'),
    JSON.stringify(
  ...
  const configDrivenRes = spawnSync(process.execPath, [prdBin, 'roll', 'dispatch', '--dry-run'], {
    cwd: tmp,
    encoding: 'utf8',
  });
  assert.equal(configDrivenRes.status, 0, (configDrivenRes.stderr || '') + (configDrivenRes.stdout || ''));

  const cliOverrideRes = spawnSync(process.execPath, [prdBin, 'roll', 'dispatch', '--agent', 'codex', '--dry-run'], {
    cwd: tmp,
    encoding: 'utf8',
  });
  assert.equal(cliOverrideRes.status, 0, (cliOverrideRes.stderr || '') + (cliOverrideRes.stdout || ''));
...
test('prd CLI supports project map add and list', async () => {
  ...
  const repoRoot = process.cwd();
  const mappedRepo = await createTempRepoDir('prd-cli-map-repo-');
  ...
  const addRes = spawnSync(
    process.execPath,
    [prdBin, 'project', 'map', 'add', '--hub', tmp, '--project', 'p2', '--repo-path', mappedRepo, '--non-interactive'],
    { encoding: 'utf8' },
  );
  ...
  assert.match(listRes.stdout || '', new RegExp(`- p2: ${mappedRepo.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`));
});

test('prd project add rejects missing local repo paths', async () => {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'prd-cli-project-missing-repo-'));
  await setupTempCliRepo(tmp);
  await fs.writeFile(path.join(tmp, 'AGENT.md'), '# guide\n', 'utf8');
  await fs.writeFile(path.join(tmp, 'PROJECTS.json'), '{"projects":{}}\n', 'utf8');
  await mkdirp(path.join(tmp, 'projects'));
  ...
  const prdBin = path.join(tmp, 'bin', 'prd.mjs');
  const missingRepo = path.join(tmp, 'does-not-exist');

  const res = spawnSync(
    process.execPath,
    [prdBin, 'project', 'add', '--hub', tmp, '--project', 'p-missing', '--repo-path', missingRepo, '--non-interactive'],
    { encoding: 'utf8' },
  );

  assert.notEqual(res.status, 0);
  assert.match((res.stderr || '') + (res.stdout || ''), /directory does not exist/i);
});

test('prd project map add rejects missing local repo paths', async () => {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'prd-cli-map-missing-repo-'));
  await setupTempCliRepo(tmp);
  await fs.writeFile(path.join(tmp, 'AGENT.md'), '# guide\n', 'utf8');
  await fs.writeFile(path.join(tmp, 'PROJECTS.json'), '{"projects":{}}\n', 'utf8');
  await mkdirp(path.join(tmp, 'projects'));
  ...
  const res = spawnSync(
    process.execPath,
    [prdBin, 'project', 'map', 'add', '--hub', tmp, '--project', 'p-missing', '--repo-path', missingRepo, '--non-interactive'],
    { encoding: 'utf8' },
  );
