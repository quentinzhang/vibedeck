...
test('computeWorktreeNames namespaces by project to avoid collisions', () => {
  ...
  assert.equal(a.branchName, 'vbd/p1/FEAT-0001');
  assert.equal(b.branchName, 'vbd/p2/FEAT-0001');
});
...
test('parseGitWorktreeListPorcelain extracts worktree path and branch', () => {
  const sample =
    ...
    'branch refs/heads/vbd/p1/FEAT-0001\n' +
    '\n' +
    'worktree /tmp/repo\n' +
  ...
  assert.equal(entries[0].branch, 'vbd/p1/FEAT-0001');
